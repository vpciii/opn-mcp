...
- Any "WAN-origin firewall blocks" warning where count >= 500 (real flood)
- Any "pf state table" warning where percent >= 90 (critical)

**MEDIUM** (notify any time, 24/7):
- "Certificate ... expires in N d" where 7 <= N < 30
- "WAN-origin firewall blocks" where 200 <= count < 500
- "pf state table" where 70 <= percent < 90

**LOW** (notify only between 08:00 and 23:00 local time):
- "Pending OPNsense / package updates"
- Any other warnings not matched above (excluding WAN blocks — see suppress rule below)

**Suppress entirely** (no notification at any time):
- "WAN-origin firewall blocks" warning where count < 200 — this is baseline internet noise (scanners, IoT botnet recon) that the firewall is correctly blocking; the daily summary reports the count for visibility but there's nothing actionable in the moment

### 4. Apply suppression + quiet hours

First, drop any warnings matching the "suppress entirely" rule above. If the remaining warning list is empty, exit silently.

Get the current local time (America/New_York). If the current hour is < 8 or >= 23 AND **all** remaining warnings are LOW severity, exit silently — they'll be picked up by the next run after 08:00.
...
- Examples:
  ...
  - `⚠️ 2 MEDIUM · cert 12d, WAN blocks 285`
...
- Run at 02:00, warnings = ["92 WAN-origin firewall blocks"] → suppressed (under 200) → silent
...
- Run at 09:00, warnings = ["230 WAN-origin firewall blocks"] → MEDIUM → notify
